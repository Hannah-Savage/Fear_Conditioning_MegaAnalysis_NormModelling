...
perm_dir = os.path.join(w_dir,'Validation/Contingency_Awareness_permutations/')
...
true_mean = np.load(os.path.join(w_dir +'Validation/Contingency_Awareness_mean_auc.npy'))
true_mean = true_mean.mean()

perm_means =  []

for i in range(1,101):
    filename = os.path.join(perm_dir +'Contingency_Awareness_mean_auc_all_permutations_'+str(i) +'.npy')
    print(filename)
